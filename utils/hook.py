...
class HookManager(object):
    ...
    def register_single(self, name: Text, func: Callable): # for single
        assert name
        found_successor = False
        for header, d in self.forks.items():
            if name.startswith(header + '.'):
                next_ = name[len(header) + 1:].split('.')[0]
                prev_ = header.split('.')[0]
                if next_.isnumeric() and  prev_ + '.' + next_ == header:
                    d.register_single(name[len(header)+1:], func)
                elif next_ == '*':
                    d.register_single(name[len(prev_ + '.*')+1:], func)
                else:
                    d.register_single(name[len(header)+1:], func) 
                found_successor = True
        if not found_successor:
