...
from utils.transformer_edit import VisionTransformerEdit, TextTransformerEdit
...
def convert_weights_to_lp(model: nn.Module, dtype=torch.float16):
    ...
    def _convert_weights(l):
        ...
        if isinstance(l, (CLIP, TextTransformer,TextTransformerEdit)):
            # convert text nn.Parameter projections
            attr = getattr(l, "text_projection", None)
            if attr is not None:
                attr.data = attr.data.to(dtype)

        if isinstance(l, VisionTransformer,VisionTransformerEdit):
...
class CLIP(nn.Module):
    ...
    def __init__(
        ...
        self.visual = _build_vision_tower(embed_dim, vision_cfg, quick_gelu, cast_dtype, self.hook_manager.fork('visual'),edit_mode,layers_to_hook,hook_type = hook_type)

        text = _build_text_tower(embed_dim, text_cfg, quick_gelu, cast_dtype, self.hook_manager.fork('textual'),edit_mode)
