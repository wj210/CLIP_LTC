...
class MultiheadAttention(nn.Module):
    ...
    def forward_per_head(self, x, attn_mask=None):
        ...
        if self.hook is not None:
            x = self.hook(
                ...
                ) # we sum over the key dimension since we will insert the summed key
            ) 
            # x = x.sum(axis=2) # bias already added in direct
        else:
            x = torch.einsum("bnmhc,dhc->bnmhd",x,self.out_proj.weight.reshape(self.embed_dim, self.num_heads, self.head_dim)).sum(axis=(2,3))
    ...
    def forward(self, x, attn_mask=None, method: Text = "ov_circuit"):
        if method == "direct":
            ...
            x = self.forward_per_head(x, attn_mask=attn_mask) # using this, just edit this
...
class ResidualAttentionBlock(nn.Module):
    def __init__(
        ...
        norm_layer: Callable = LayerNorm,
        hook: Optional[HookManager] = None,
        hook_type: str = "attn", # either all, attn or mlp
    ):
        super().__init__()
        self.hook = hook
        if self.hook is not None and (hook_type in ['all','attn']):
        ...
        if self.hook is not None and (hook_type in ['all','mlp']):
    ...
    def forward(
        ...
        x = q_x + self.ls_1(after_attn) # residual connection for attn, if indirect we keep x as same, but change the x going into mlp. total change both
        if self.hook is not None:
            x_intermediate = self.hook('after_attn',ret = x,residual=q_x) # if indirect we change this to the corrupted, if total just keep it same.
        else:
            x_intermediate = x
        after_ln2 = self.ln_2(x_intermediate)
        after_mlp = self.mlp(after_ln2)
        # after_mlp = self.hook("after_mlp", ret=after_mlp) 
        x = x + self.ls_2(after_mlp) # indirect causes the change via after_mlp while x residual stays the same.
    ...
    def __init__(
        ...
        self.resblocks = nn.ModuleList(
            [
                ...
                ResidualAttentionBlock( # no need to hook those not intervening
                    width,
                    heads,
                    mlp_ratio,
                    ls_init_value=ls_init_value,
...
class VisionTransformerEdit(nn.Module):
...
class TextTransformerEdit(nn.Module):
    output_tokens: torch.jit.Final[bool]
